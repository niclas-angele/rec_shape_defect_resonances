function xp=searchxpdroite(v,mesh,xs,xmax)
%%% function to use the data and localize an approximation of x^star_k
%%% using data with a source on the left of the waveguide. v is the total
%%% wavefield, mesh its associated mesh, xs the position of the source and
%%% xmax the maximum of the search interval 
    % search interval 
    z=linspace(xs,min(xs+20,xmax),300); 
    ...
    z=linspace(max(xapprox-2,xs),min(xapprox+2,xmax),300); 
    tempz=P1togrid(mesh,v,z,0);
    %minimization
    g=@(y) log(norm((y(1)+1i*y(4))*airy(y(2)*(-y(3)+z))-tempz))+1000*(y(3)<xs);
